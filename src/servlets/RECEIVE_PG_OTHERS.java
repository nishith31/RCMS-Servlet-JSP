...
/*LOGIC FOR CHECKING THE EXISTENCE OF THE ENTRIES TO BE MADE IN DATABSE ALREADY*/   
    ...
    {
        first=stmt.executeQuery("select * from others_receive_"+current_session+"_"+regionalCenterCode+" where crs_code='"+courses[i]+"' and block='PG' and date='"+date+"'");
        if(first.next())
        {
            flag=1;
...
/*LOGIC ENDS HERE FOR CHEKING THE EXISTENCE OF THE ENTRIES TO BE MADE*/
    if(flag==0)
    {
        ...
        for(int i=0;i<courses.length;i++)
        {
            result_receive=stmt.executeUpdate("insert into others_receive_"+current_session+"_"+regionalCenterCode+"(crs_code,block,qty,medium,date,receive_from) values('"+courses[i]+"','PG',"+qtys[i]+",'"+mediums[i]+"','"+date+"','"+receive_from+"')");
            result_material=stmt.executeUpdate("update material_"+current_session+"_"+regionalCenterCode+" set qty=qty+"+qtys[i]+" where crs_code='"+courses[i]+"' and block='PG' and medium='"+mediums[i]+"'");
