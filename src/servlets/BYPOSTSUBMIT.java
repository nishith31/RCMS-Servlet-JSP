...
/*logic of getting the ABSOLUTE PROGRAMME CODE*/
    ...
    {
        int o=stmt.executeUpdate("insert into student_dispatch_"+current_session+"_"+rc_code+"(enrno,prg_code,crs_code,block,qty,date,dispatch_source,medium,challan_no,pkt_weight,pkt_type,reentry,express_no)values('"+enrollmentNumber+"','"+prg_code+"','"+prg_code+"','PG',1,'"+date+"','"+dispatch_source+"','"+medium+"','"+challan_no+"',"+pkt_weight+",'"+pkt_type+"','NO','"+express_no+"')");   
    ...
    if (block_count != 0) 
    {
        ...
        for(int z=0;z<course.length;z++)
        {
            ...
            for(int y=0;y<course_dispatch.length;y++)
            {
                ...
                if(course[z].equals(course_check))
                {
                    if(initial.equals("B"))
                    {
                        rs=stmt.executeQuery("select qty from material_"+current_session+"_"+rc_code+" where crs_code='"+relativeCourse+"' and block='"+block_check+"' and medium='"+medium+"'");
                        while(rs.next())
            ...
            for(int z=0;z<course.length;z++) {
                ...
                for(int y=0;y<course_dispatch.length;y++) {
                    ...
                    if(course[z].equals(course_check))
                    {
                        if(initial.equals("B"))
                        {
                            result=stmt.executeUpdate("insert into student_dispatch_"+current_session+"_"+rc_code+"(enrno,prg_code,crs_code,block,qty,date,dispatch_source,medium,challan_no,pkt_weight,pkt_type,reentry,express_no)values('"+enrollmentNumber+"','"+prg_code+"','"+course[z]+"','"+block_check+"',1,'"+date+"','"+dispatch_source+"','"+medium+"','"+challan_no+"',"+pkt_weight+",'"+pkt_type+"','NO','"+express_no+"')");   
                            result1=stmt.executeUpdate("update material_"+current_session+"_"+rc_code+" set qty=qty-1 where crs_code='"+relativeCourse+"' and block='"+block_check+"' and medium='"+medium+"'");
                        }//end of if(initial.equals("B"))
                    }//end of if(course[z].equals(course_check))
                }//end of for loop of y
            }//end of for loop z    
        if(result==1 && result1==1) {   
            ...
            message=""+course_dispatch.length+" Books Despatched to "+name+".";
    ...
    request.setAttribute("msg",message);
    request.getRequestDispatcher("jsp/By_post.jsp").forward(request,response);
    System.out.println("exception mila rey from BYPOSTSUBMIT.java"+exception);
}//end of catch blocks
