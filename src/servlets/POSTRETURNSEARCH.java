package servlets;
/*THIS SERVLET IS RESPONSIBLE FOR SEARCHING THE PERSONAL DETAILS AND DESPATCH DETAILS OF TEH STUDENT WHOSE ROLL NO HAS BEEN ENTERED IN THE BROWSER,IF NO DESPATCH DETAILS FOUND THEN REDIRECTS TO THE SAME PAGE AND IF DETAILS FOUND THEN REDIRECTS TO THE FROM_POST1.JSP FROM WHERE RECEVING WILL BE STARTED.
...
/*LOGIC FOR CHECKING THE DESPATCH OF PROGRAMME GUIDE TO THE STUDENT*/
	rs=stmt.executeQuery("select * from student_dispatch_"+current_session+"_"+rc_code+" where enrno='"+enrno+"' and block='PG' and dispatch_source='BY POST'");
	if(rs.next())
	{
		pg_flag				=	"YES";
...
/*LOGIC ENDS HERE FOR CHECKING THE DESPATCH OF PROGRAMME GUIDE*/
		...
		rs=stmt.executeQuery("select * from student_"+current_session+"_"+rc_code+" where enrno='"+enrno+"'");
			...
			{	prog = rs.getString(2);			
				...
				rs=stmt.executeQuery("select count(distinct(crs_code)) from student_dispatch_"+current_session+"_"+rc_code+" where enrno='"+enrno+"' and block<>'PG' and dispatch_source='BY POST'");
				while(rs.next())
				{
					dispatch_course_count=rs.getInt(1);
				}
			String course_dispatch[]		=	new String[dispatch_course_count];
			rs=stmt.executeQuery("select distinct(crs_code) from student_dispatch_"+current_session+"_"+rc_code+" where enrno='"+enrno+"' and block<>'PG' and dispatch_source='BY POST'");
			index=0;
			while(rs.next())
			{
			...
			index=0;
			rs=stmt.executeQuery("select count(*) from student_dispatch_"+current_session+"_"+rc_code+" where enrno='"+enrno+"' and block<>'PG' and dispatch_source='BY POST'");
				while(rs.next())
				{
					index=rs.getInt(1);
				}//index++;
