...
try
{
	...
	for(int i=0;i<courses.length;i++)
	{
		...
		for(int j=0;j<blk_count;j++)
		{
			count=j+1;
			blocks[j]="B"+count;
		first=stmt.executeQuery("select * from student_receive_"+current_session+"_"+rc_code+" where enrno='"+enrno+"' and crs_code='"+courses[i]+"' and block='"+blocks[j]+"'");
			if(first.next())
			{	
				flag=1;
//				System.out.println("Entry Already Exist for Course "+courses[i]+" Block "+blocks[j]+" for Enrollment No. "+enrno+"");   
	//			System.out.println(""+courses[i]+" ne wapas bheja");
				msg=msg+courses[i]+" Block "+blocks[j]+"<br/>";						
			}//end of if first
...
/*LOGIC FOR INSERTING THE MATERIAL INTO RECEIVE DATABASE IF FLAG==0 MEANS DUPLICATE ENTRY NOT FOUND ALREADY*/
	if(flag==0)
	{
		...
		for(int i=0;i<courses.length;i++)
		{
			...
			for(int j=0;j<no_of_blocks[i];j++)
			{
				...
				blocks[j]="B"+count;
				result_receive=stmt.executeUpdate("insert into student_receive_"+current_session+"_"+rc_code+"(enrno,prg_code,crs_code,block,qty,medium,date,receive_source) values('"+enrno+"','"+prg_code+"','"+courses[i]+"','"+blocks[j]+"',"+qtys[i]+",'"+medium+"','"+date+"','"+receive_source+"')");
	...
	{
	msg="Welcome to The Partial Receipt of Materials from "+name+".<br/>";
		for(int i=0;i<courses.length;i++)
		{
			block=stmt.executeQuery("select no_of_blocks from course where crs_code='"+courses[i]+"'");
			while(block.next())
		...
		request.setAttribute("msg",msg);//sending the output message to the browser
