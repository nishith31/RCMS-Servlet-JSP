package servlets;
/*THIS SERVLET IS RESPONSIBLE FOR SEARCHING ALL THE DETAILS OF THE STUDENT FROM THE STUDENT DATABASE AND DISPLAY THEM TO THE NEXT PAGE.IF ANY COURSE HAS NOT BEEN DESPATCHED THEN ALL THE CECKBOXES WILL BE ACTIVE AND IF SOME COURSES ALREADY DESPATCHED THEN THOSE COURSES WILL BE SHOWN IN DISABLED FORMAT IN THE BROWSER.*/
...
/*Complete Logic for fetching the details of the student from the the student database*/
	...
	/*Logic for getting all the courses name, no of blocks of them and the serial number associated with the course*/
		...
		int i=17,j=18,total_length=0,count=0;
		...
		while(rs.next())
		{
			for(index=0;index<course_number;index++)
			{
				...
				{						//System.out.println("course found in course vs course  table");					
...
/*Logic for checking the Despatch of Programme guide for the student entered into the system*/		
	rs=stmt.executeQuery("select * from student_dispatch_"+current_session+"_"+rc_code+" where enrno='"+enrno+"' and "+search_crs_code+" and block='PG' and reentry='YES'");
		if(rs.next())
		{
			pg_flag="YES";//if program guide is already despatched then flag will be YES
...
/*Logic for getting the available sets of the Programme guide of the program opted by the student*/		
		...
		rs=stmt.executeQuery("select count(*) from student_dispatch_"+current_session+"_"+rc_code+" where enrno='"+enrno+"' and block<>'PG' and reentry='YES'");
		while(rs.next())
		{
...
/*If Not any course opted by student has been Despatched then this logic will create empty Despatch course and date array*/
		...
		if(dispatch_course_count==0)
		{
			...
			msg=msg+"Welcome to the RE-ENTRY By Hand Entry for "+name+".<br/>";
/*Sending the Despatch courses and dates empty array with the msg variable with appropriate msg to the Browser*/		
			...
			request.setAttribute("msg",msg);
			request.getRequestDispatcher("jsp/By_hand3.jsp?enrno="+enrno+"&prog="+prog+"&year="+year+"&name="+name+"&medium="+medium+"&available_pg="+available_pg+"").forward(request,response); 
...
/*If some courses partially Despatched then this logic will create the Despatched courses and dates array and sent to client*/
		...
		String checking=null;
			...
			rs=stmt.executeQuery("select * from student_dispatch_"+current_session+"_"+rc_code+" where enrno='"+enrno+"' and block<>'PG' and reentry='YES'");
			while(rs.next())
			{
...
/*If some courses partially Despatched then this logic will create the Despatched courses and dates array and sent to client*/
		...
		int m=0,counter=0;
			...
			m=0;
			msg=msg+"Partially Despatched Courses of "+name+" are ";
			for(int k=0;k<dispatch.length;k++)
			{
				msg=" "+msg+" "+dispatch[k];
			}
		...
		else
		{
			...
			request.getRequestDispatcher("jsp/By_hand.jsp?enrno="+enrno+"&prog="+prog+"&year="+year+"&name="+name+"&medium="+medium+"&date="+date+"&available_pg="+available_pg+"&dispatch_mode="+dispatch_mode+"").forward(request,response);
		}//end of else
...
}//end of class BYHANDREENTRYSEARCH