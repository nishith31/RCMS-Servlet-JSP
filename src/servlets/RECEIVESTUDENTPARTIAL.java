package servlets;
/*THIS SERVLET IS RESPONSIBLE FOR INSERTING DATA TO STUDENT RECEIVE TABLE AND UPDATING MATERIAL TABLE.IT ALSO CHECKS THE VIOLATION OF PRIMARY KEY MEANS DUPLICATE DATA CAN NOT BE ENTER IN THE STUDENT RECEIVE TABLE.THIS SERVLET GETS ALL THE REQUIRED FIELDS FROM THE BROWSER AND AFTER CHECKING ALL THE CONSTRAINTS INSERT AND UPDATE THE CORRESPONDING TABLES.
 ...
 public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
{
	...
	String enrno			=	 request.getParameter("txt_enr").toLowerCase();//getting the value of enrollment number
	String name				=	 request.getParameter("txt_name").toLowerCase();//getting the value of name
...
try
{
	...
	{
		qty=block_count;
		msg="Entry Already Exist for Enrollment No "+enrno+" for Course: <br/>";
		for(int z=0;z<course.length;z++)
		{
			...
			for(int y=0;y<course_dispatch.length;y++)
			{
				...
				if(course[z].equals(course_check))
				{
					if(initial.equals("B"))
					{
						rs=stmt.executeQuery("select * from student_receive_"+current_session+"_"+rc_code+" where enrno='"+enrno+"' and crs_code='"+course[z]+"' and block='"+block_check+"'");
						if(rs.next())
						{
							msg=msg+course[z]+" Block "+block_check+"<br/>";
							flag_for_return=1;
						}//end of if
		...
		{
			msg="Received Successfully from STUDENT Course <br/>";
			int inc=0;
			for(int z=0;z<course.length;z++)
			{
				int len=course[z].length();
				for(int y=0;y<course_dispatch.length;y++)
				{
					...
					if(course[z].equals(course_check))
					{
						if(initial.equals("B"))
						{
							result=stmt.executeUpdate("insert into student_receive_"+current_session+"_"+rc_code+"(enrno,prg_code,crs_code,block,qty,medium,date,receive_source) values('"+enrno+"','"+prg_code+"','"+course[z]+"','"+block_check+"',"+blk_qty[inc]+",'"+medium+"','"+date+"','"+receive_source+"')");   
...
{
	System.out.println("exception mila rey from RECEIVESTUDENTPARTIAL.java and exception is "+exe);
