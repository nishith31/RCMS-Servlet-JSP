        ...
        for(int z=0;z<course.length;z++)
        {
            ...
            for(int y=0;y<course_dispatch.length;y++)
            {
                ...
                if(course[z].equals(course_check))
                {
                    if(initial.equals("B"))
                    {
                        rs=stmt.executeQuery("select * from student_receive_"+currentSession+"_"+rc_code+" where enrno='"+enrno+"' and crs_code='"+course[z]+"' and block='"+block_check+"'");
                        if(rs.next())
                        {
            ...
            for(int z=0;z<course.length;z++)
            {
                ...
                for(int y=0;y<course_dispatch.length;y++)
                {
                    ...
                    if(course[z].equals(course_check))
                    {
                        if(initial.equals("B"))
                        {
                            result=stmt.executeUpdate("insert into student_receive_"+currentSession+"_"+rc_code+"(enrno,prg_code,crs_code,block,qty,medium,date,receive_source) values('"+enrno+"','"+programmeCode+"','"+course[z]+"','"+block_check+"',"+blk_qty[inc]+",'"+medium+"','"+date+"','"+receive_source+"')");   
