package servlets;
/*THIS SERVLET IS RESPONSIBLE FOR INSERTING DATA TO RC RECEIVE TABLE AND UPDATING MATERIAL TABLE.IT ALSO CHECKS THE VIOLATION OF PRIMARY KEY MEANS DUPLICATE DATA CAN NOT BE ENTER IN THE RC RECEIVE TABLE.THIS SERVLET GETS ALL THE REQUIRED FIELDS FROM THE BROWSER AND AFTER CHECKING ALL THE CONSTRAINTS INSERT AND UPDATE THE CORRESPONDING TABLES.
		...
		for(int z=0;z<course.length;z++)
		{
			...
			for(int y=0;y<course_dispatch.length;y++)
			{
				...
				if(course[z].equals(course_check))
				{
					if(initial.equals("B"))
					{
						rs=stmt.executeQuery("select * from rc_receive_"+current_session+"_"+rc_code+" where reg_code='"+reg_code+"' and crs_code='"+course[z]+"' and block='"+block_check+"' and medium='"+medium+"' and date='"+date+"'");
						if(rs.next())
						{
			...
			for(int z=0;z<course.length;z++)
			{
				...
				for(int y=0;y<course_dispatch.length;y++)
				{
					...
					if(course[z].equals(course_check))
					{
						if(initial.equals("B"))
						{
							result=stmt.executeUpdate("insert into rc_receive_"+current_session+"_"+rc_code+"(reg_code,crs_code,block,qty,medium,date)values('"+reg_code+"','"+course[z]+"','"+block_check+"',"+blk_qty[inc]+",'"+medium+"','"+date+"')");   
