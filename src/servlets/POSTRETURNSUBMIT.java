    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(session == null) {
            ...
            try {
                ...
                if(!rs.next()) {
                    rs  =   stmt.executeQuery("select absolute_prg_code from program_program where relative_prg_code='"+programmeCode+"' and rc_code='"+regionalCenterCode+"'");
                    if(rs.next()) {
                ...
                if(programmeGuideFlag.equals(Constants.YES) && programmeGuideValue!=null ) {
                    ...
                    programmeGuideResult = stmt.executeUpdate("insert into student_receive_"+currentSession+"_"+regionalCenterCode+" values('"+enrno+"','"+programmeCode+"','"+programmeCode+"','PG',1,'"+programmeGuideMedium+"','"+date+"','"+receiveSource+"','"+receiveRemarks+"','"+programmeGuideExpress+"','"+programmeGuideDate+"')");   
                    programmeGuideResult1 = stmt.executeUpdate("update material_"+currentSession+"_"+regionalCenterCode+" set qty=qty+1 where "+actualProgrammeCode+" and block='PG' and medium='"+programmeGuideMedium+"'");
                    programmeGuideResult2 = stmt.executeUpdate("delete from student_dispatch_"+currentSession+"_"+regionalCenterCode+" where enrno='"+enrno+"' and "+actualProgrammeCode+" and block='PG' and dispatch_source='BY POST' and express_no='"+programmeGuideExpress+"'");
                    if(programmeGuideResult > 0 && programmeGuideResult1 > 0 && programmeGuideResult2 > 0) {
                        message = "Material of Programme Guide Updated successfully Returned from Post for " + programmeCode + 
                                " <br/> Roll No." + enrno + "<br/>Name. " + name + "<br/>";
                    } else {
                ...
                if (course.length>0) {
                                    ...
                                    if(course[index].trim().equals(courseCheck)) {
                                        if(initial.trim().equals("B")) {
                                            result += stmt.executeUpdate("insert into student_receive_" + currentSession + Constants.UNDERSCORE +
                                                    regionalCenterCode + " values('" + enrno + "','" + programmeCode + "','" + course[index] + "','"
                                            ...
                                            result2 += stmt.executeUpdate("delete from student_dispatch_" + currentSession + Constants.UNDERSCORE +
                                                    regionalCenterCode + " where enrno='" + enrno + "' and crs_code='" + course[index] + "' and block='"
                    ...
                    if(result==blockCount && result1==blockCount && result2==blockCount) {
                        message = message + "Material Updated Successfully Returned from Post for " + blockCount + 
                                " Blocks <br/> Roll No." + enrno + "<br/>Name. " + name+"<br/>";
                    } else {
