        ...
        for(int z=0;z<course.length;z++)
        {
            ...
            for(int y=0;y<course_dispatch.length;y++)
            {
                ...
                if(course[z].equals(course_check))
                {
                    if(initial.equals("B"))
                    {
                        rs=stmt.executeQuery("select * from mpdd_receive_"+currentSession+"_"+rc_code+" where crs_code='"+course[z]+"' and block='"+block_check+"' and medium='"+medium+"' and date='"+date+"'");
                        if(rs.next())
                        {
            ...
            for(int z=0;z<course.length;z++)
            {
                ...
                for(int y=0;y<course_dispatch.length;y++)
                {
                    ...
                    if(course[z].equals(course_check))
                    {
                        if(initial.equals("B"))
                        {
                            result=stmt.executeUpdate("insert into mpdd_receive_"+currentSession+"_"+rc_code+"(crs_code,block,qty,medium,date)values('"+course[z]+"','"+block_check+"',"+blk_qty[inc]+",'"+medium+"','"+date+"')");   
