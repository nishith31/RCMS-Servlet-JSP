 ...
 public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
{
	...
	String sc_code			= 	 request.getParameter("mnu_sc_code").toUpperCase();//gettting the prgram code
		...
		for(int z=0;z<course.length;z++)
		{
			...
			for(int y=0;y<course_dispatch.length;y++)
			{
				...
				if(course[z].equals(course_check))
				{
					if(initial.equals("B"))
					{
						rs=stmt.executeQuery("select * from sc_receive_"+current_session+"_"+rc_code+" where sc_code='"+sc_code+"' and crs_code='"+course[z]+"' and block='"+block_check+"' and medium='"+medium+"' and date='"+date+"'");
						if(rs.next())
						{
			...
			for(int z=0;z<course.length;z++)
			{
				...
				for(int y=0;y<course_dispatch.length;y++)
				{
					...
					if(course[z].equals(course_check))
					{
						if(initial.equals("B"))
						{
							result=stmt.executeUpdate("insert into sc_receive_"+current_session+"_"+rc_code+"(sc_code,crs_code,block,qty,medium,date)values('"+sc_code+"','"+course[z]+"','"+block_check+"',"+blk_qty[inc]+",'"+medium+"','"+date+"')");   
